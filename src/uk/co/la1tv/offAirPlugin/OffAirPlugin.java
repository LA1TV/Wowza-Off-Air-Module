...
public class OffAirPlugin extends ModuleBase {
	...
	public void onAppStart(IApplicationInstance appInstance) {
		...
		timeToShowOffAirVideoFor = appInstance.getProperties().getPropertyInt(PROP_NAME_PREFIX+"timeToShowOffAirVideoFor", 600);
	...
	private class StreamListener implements IMediaStreamActionNotify {
		...
		public void onPublish(IMediaStream mediaStream, String streamNameParam, boolean isRecord, boolean isAppend) {
			if (mediaStream.isPublisherStream()) {
				// this is a server side stream, which may be a stream generated by the transcoder
				return;
			}
	...
	private class TranscoderControl implements ILiveStreamTranscoderControl {
		...
		// returning false will disable the transcoder for the stream
		public boolean isLiveStreamTranscode(String transcoder, IMediaStream mediaStream) {
			String streamName = mediaStream.getName();
			if (mediaStream.isTranscodeResult() || !isOutgoingStreamName(streamName)) {
				// don't transcode if this is an output from the transcoder,
				// or if this is an incoming stream
				return false; 
			}
			else {
				return true;
